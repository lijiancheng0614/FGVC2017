...
#include "caffe/layers/image_multi_label_data_layer.hpp"
...
void ImageMultiLabelDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO) << "Expected " << line_cnt << " images with " << label_len << " labels.";
  vector<float> labels(label_len);
...
void ImageMultiLabelDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    const vector<float> labels = lines_[lines_id_].second;
    int label_len = labels.size();
    for (int j = 0; j < label_len; ++j) {
      prefetch_label[item_id * label_len + j] = (Dtype)labels[j];
    }
    // go to the next iter
    lines_id_++;
